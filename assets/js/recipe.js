(function () {
  ...
  recipe.bindAdjuster = function () {
    recipe.rootNode.innerHTML = recipe.rootNode.innerHTML.replace(
      /(serves )(\d+)/i,
      function (unused_match, serves, quantity) {
        return (
          ...
          '" step="' +
          quantity / 4 +
          '" onchange="recipe.adjustIngredients(this.value/' +
          quantity +
          ')"/>'
        );
      }
  ...
  recipe.markIngredients = function () {
    var ingredients = recipe.rootNode.querySelectorAll(
  ...
  recipe.markIngredient = function (ingredient) {
    ingredient.innerHTML = ingredient.textContent.replace(
      ...
      function (match, quantity, unit) {
        ...
        return (
          '<span class="ingredient" data-quantity="' +
  ...
  recipe.adjustIngredient = function (ingredient, scale) {
    ...
    ingredient.textContent = new recipe.Ingredient(scaledQuantity, unit)
      .convertUnits()
      .toString();
  };
